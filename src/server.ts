...
function getTimeRange(days: number): { startMs: string; endMs: string } {
  const end = Date.now();
...
export const getServer = (): McpServer => {
  const server = new McpServer(
    { name: "google-fit-mcp", version: "0.1.0" },
    { capabilities: {} },
  );
  ...
  server.registerTool(
    ...
    async ({ days }): Promise<CallToolResult> => {
      ...
      const sleepTypes: Record<number, string> = {
      ...
      for (const pt of res.data.point ?? []) {
        ...
        if (type !== "Sveglio") totalMin += durMin;
        ...
        for (const ds of bucket.dataset ?? [])
          for (const pt of ds.point ?? []) {
            ...
            const durMin = Math.round(
              (Number(pt.endTimeNanos) - Number(pt.startTimeNanos)) / 60e9
            );
